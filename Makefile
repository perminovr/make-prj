...
.PHONY: prebuild_clean clean cleanall prebuild
...
prebuild: $(SYSROOT_LOG) $(SYSROOT_WD) $(BUILD_LOG) $(BUILD_INC) $(BUILD_LIB) $(PRE_CSO_MkFiles) $(PRE_CBIN_MkFiles) $(PRE_CPPSO_MkFiles) $(PRE_CPPBIN_MkFiles)
	@echo "Prebuild ok"
	@touch $(BUILD_DIR)/prebuild_ok

$(BUILD_INC):
	@mkdir -p $(BUILD_INC)
	@cp -r include/* $(BUILD_INC)
...
prebuild_clean: $(CSO_CLEAN) $(CBIN_CLEAN) $(CPPBIN_CLEAN) $(CPPSO_CLEAN)
	@rm -rf $(BUILD_DIR)/prebuild_ok

clean:
	@if [ -f $(BUILD_DIR)/prebuild_ok ]; then $(MAKE) prebuild_clean ; fi
